...
class MY_Model extends Base_Model {
	...
	public function setProperty($data = null){ 
		if(isset($data)):
			foreach($data as $key => $value):
				if(property_exists(get_class($this), $key) || $this->$key):
					$this->$key = $value;
				endif;
			endforeach;
		endif;
	...
	protected function getIdUser(){
		if($this->session->userdata('mfa_temp')){
			$this->ion_auth->logout();	
			redirect($this->router->fetch_module(). $mLoginUrl);
		}
